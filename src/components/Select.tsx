...
export const Select: FC<SelectProps> = ({ label, value, options, disabled, onChange }) => {
    ...
    >
      <button
        ...
        className={classnames(
          'rounded border border-innovasive-ui-grey-medium w-full h-40 px-24 focus:outline-none',
          {
          ...
          <ul>
            {options?.map((o) => (
              <li
                ...
                className={classnames('h-40 flex items-center px-24 cursor-pointer', {
                  'bg-innovasive-ui-grey-medium hover:bg-innovasive-ui-grey-medium':
                    value === o.value,
                  'hover:bg-innovasive-ui-grey-light': value !== o.value,
                })}
              >
                <span>{o.value}</span>
              </li>
...
export type SelectForkmikProps = {
  name: string
  options: SelectOption[]
  value: string
  label?: string
...
export const SelectFormik: FC<SelectForkmikProps> = (props: SelectForkmikProps) => {
  return (
    <Field name={props.name}>
      {({ field, meta }: any) => <Select {...field} error={meta.error} {...props} />}
    </Field>
  )
}
